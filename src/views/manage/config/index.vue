    ...
    <el-form :model="settingStore" ref="formRef" label-width="110px">
      <el-collapse v-model="activeNames">
        <el-collapse-item title="全局快捷键" name="1">
          <el-form-item v-for="shortcut in globalShortcutList" :key="shortcut.id" :label="shortcut.label">
            <ShortcutInput v-model="shortcut.shortcut" :exclude-id="shortcut.id" />
          </el-form-item>
        </el-collapse-item>
...
const globalShortcutList = computed(() => {
  return shortcutStore.shortcuts.filter(item => item.type === 'global');
});
